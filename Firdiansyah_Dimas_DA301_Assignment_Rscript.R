...
# Removing the row with Rank 1 (prod=107) due to it being the best-selling game
# This exclusion is to clarify the sales data, focusing on games with global 
# sales bellow £40M , as that is made the prod 107 potentially an outlier.
t_sales_sub <- subset(t_sales, Global_Sales <= 40)

# Remove unnecessary columns. 
t_sales_nw <- select(t_sales_sub, -c('Ranking','Year','Genre','Publisher'))
...
ggplot(t_sales_nw, aes(x = Platform, y = Global_Sales)) +
  ...
  ggtitle("Global comparison by platform")

ggplot(t_sales_nw, aes(x = Platform, y = Global_Sales)) +
  geom_line(fill = "lightblue", color = "blue") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  ggtitle("Global comparison by platform")
...
ggplot(top_10_products, aes(x = Platform, y = Global_Sales)) +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  ggtitle("Top Global sales by platform")

## Additional analysis
# Publisher count
publisher_counts <- table(t_sales$Publisher)

# Convert the publisher_counts table to a data frame
publisher_data <- as.data.frame(publisher_counts)

# Rename the columns for clarity
colnames(publisher_data) <- c("Publisher", "Count")

# Sort the data frame by count in descending order
publisher_data <- publisher_data[order(-publisher_data$Count), ]

# Create a bar plot

top_publisher <- head(arrange(publisher_data, desc(count(publisher_data))), 10)

ggplot(publisher_data, aes(x = reorder(Publisher, -Count), y = Count)) +
  geom_bar(stat = "identity", fill = "dodgerblue", width = 0.7) +
  theme_minimal() +
  xlab("Publisher") +
  ylab("Count") +
  ggtitle("Sales by Publishers") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
cor_na_global
cor_eu_global
## from the value of correlation na vs global and eu vs global shows that 
## for eu vs global (0.824) and na vs global (0.912) which indicate high value
## of corelation between each region and global sales
...
ggplot(t_sales, aes(x = Platform, y = Global_Sales)) +
  ...
  labs(title = "Global Sales by Platform",
       ...
       y = "Global Sales",
       fill = "Region")
ggplot(t_sales, aes(x = Platform, y = EU_Sales)) +
  geom_bar(stat = "identity",fill = 'blue') +
  labs(title = "Europe Sales by Platform",
       x = "Platform",
       y = "European Sales",
       fill = "Region")
ggplot(t_sales, aes(x = Platform, y = NA_Sales)) +
  geom_bar(stat = "identity",fill = 'blue') +
...
# Sales Distribution for assessing the reliability of the dataset
